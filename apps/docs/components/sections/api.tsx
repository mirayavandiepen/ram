    ...
    <Section id="api" title="API">
      <div className="divide-y divide-border border-y border-border">
        {PROPS.map(([name, type, description]) => (
          <div key={name} className="grid gap-x-6 gap-y-1 py-3 sm:grid-cols-[150px_1fr]">
            <dl className="contents">
              <dt className="font-mono text-[13px] text-foreground">{name}</dt>
              <dd>
                <p className="font-mono text-[12px] text-muted">{type}</p>
                <p className="mt-0.5 text-[14px]">{description}</p>
              </dd>
            </dl>
          </div>
        ))}
